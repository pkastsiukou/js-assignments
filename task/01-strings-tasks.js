...
function encodeToRot13(str) {
    ...
    return str.split('').map(symbol => {
        ...
        const alphabetIndex = alphabet.indexOf(symbol.toUpperCase());
        if (alphabetIndex < 0) {
            return symbol;
        }
        const index = (alphabetIndex + 13) % alphabet.length;
        return isUppercase ? alphabet[index].toUpperCase() : alphabet[index].toLowerCase();
    }).join('');
}
